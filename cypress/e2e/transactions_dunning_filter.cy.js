...
// ─────────────────────────────────────────────
// transactions_dunning_filter.cy.js
// ─────────────────────────────────────────────
describe('Transactions Overview - Dunning Filter', () => {

  const dunningOptions = [
    'In dunning',
    'Completed dunning',
    'Return dunning completed',
  ];

  it('filtrira sve Dunning opcije – login jednom', () => {
    ...
    // ─── 4. Petlja kroz sve Dunning opcije ───
    dunningOptions.forEach((option) => {
      ...
      cy.get('tbody tr.mat-mdc-row').should('have.length.greaterThan', 0).then(($rows) => {
        cy.log(`✅ Dunning "${option}" – pronađeno ${$rows.length} transakcija`);
      });
